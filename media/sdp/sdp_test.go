...
`
	...
	assert.Equal(t, 36600, md.MediaName.Port.Value)
	assert.Equal(t, "audio", md.MediaName.Media)
	assert.Equal(t, []string{"RTP", "AVP"}, md.MediaName.Protos)
	assert.Equal(t, []string{"0", "101", "8", "3"}, md.MediaName.Formats)
	require.Equal(t, 4, len(md.Attributes))

	attrs := []string{psdp.AttrKeySendRecv, psdp.AttrKeyRTCPMux}
	for _, attr := range attrs {
		_, ok := md.Attribute(attr)
		assert.True(t, ok)
	}
}
